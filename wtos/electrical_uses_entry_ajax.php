...
/*
   # wtos version : 1.1
   # page called by ajax script in construction_uses_entry_ajax.php
   #
*/
...
$ajaxFilePath= $site["url-wtos"].'electrical_uses_entry_ajax.php';
$department = "Electrical";
$access_name = "Electrical uses Entry";
//
 ...
 * Lists
 */
if($os->get("WT_fetch_usesinfo_list")=="OK"&&$os->post("WT_fetch_usesinfo_list")=="OK"){
    ...
    ?>
        ...
        <table class="bp3-html-table  bp3-html-table-striped bp3-interactive " style="width: 100%">
            ...
            $query = $os->mq("SELECT * FROM item_uses 
            ...
            while($row=$os->mfa($query)){
                ...
                ?>
                <tr class="pointable usesinfo_row usesinfo_row_<?=$row['item_use_id']?>">
                    <td>
                        <div onclick="WT_fetch_usesinfo_details('<?=$row['item_use_id']?>')">
                        ...
                        <div class="text-s m-top-s">
                            ...
                            if($delete_access){?>
                                <a class="uk-text-danger"
                                   onclick="removeRowAjaxFunction('item_uses','item_use_id','<?=$row['item_use_id']?>','','','WT_fetch_usesinfo_list();') "
                            ...
                            if($edit_access){?>
                                <a class="uk-text-primary" tabindex="0"
                                   onclick="WT_open_usesinfo_form(<?=$row['item_use_id']?>)"
                                   role="button">Edit</a>
                            ...
                            <?if($show_verify_btn){?>
                                ...
                                <a class="uk-text-success" tabindex="0"
                                   onclick="WT_do_verification(<?=$row['item_use_id']?>, '<?=$verify_text?>')"
...
if($os->get("WT_fetch_usesinfo_details")=="OK"&&$os->post("WT_fetch_usesinfo_details")=="OK"){
    $item_use_id = $os->post("item_use_id");
    $usesinfo = $os->mfa($os->mq("SELECT * FROM item_uses WHERE item_use_id='$item_use_id'"));

    $primary_verified = $usesinfo["primary_verified"];
    $final_verified = $usesinfo["final_verified"];
    ?>
    <div class="item-header p-m uk-box-shadow-small" style="border-bottom: 1px solid #e5e5e5">
        <table class="uk-table uk-table-justify congested-table uk-margin-remove">
                ...
                <td>
                    <div class="text-l">usesinfo Items</div>
                    <div class="text-s uk-text-primary"><?=$usesinfo["reff_no"]?></div>
                </td>
                <td class="uk-text-right">
                        <div class="bp3-button-group .modifier">
                            <a class="bp3-button bp3-small bp3-icon-add" tabindex="0" role="button"
                               onclick="WT_open_usesinfo_details_form(<?=$item_use_id?>)">New Item</a>
                        </div>
    ...
    <div class="item-content uk-overflow-auto">
        <table class="bp3-html-table  bp3-html-table-striped bp3-interactive bp3-html-table-condensed" style="width: 100%">
            ...
            <tbody>
            <?
            $query = $os->mq("SELECT ied.*, i.item_name 
                FROM item_uses_details ied
                INNER JOIN items i ON i.item_id=ied.item_id                 
                WHERE ied.item_use_id='$item_use_id' ORDER BY item_uses_detail_id DESC");
            $x=0;
                    ...
                    <td>
                       ...
                       <!-- <div class="text-s uk-hidden@s">Quantity : <i><?=rand(1,99)?>Kg</i></div>
                        ...
                        <div class="text-s m-top-s">
                            ...
                            \
                            <a class="uk-text-primary" tabindex="0"
                               onclick="WT_open_usesinfo_details_form(<?=$item_use_id?>, <?=$record['item_uses_detail_id']?>)"
                               role="button">Edit</a>

                            <?}?>
                            <a class="uk-text-primary uk-hidden" tabindex="0"
                               onclick="WT_open_usesinfo_details_transfer_form(<?=$item_use_id?>, <?=$record['item_uses_detail_id']?>)"
 ...
 * Forms
 */
if($os->get("get_usesinfo_form")=="OK" && $os->post("get_usesinfo_form")=="OK"){
    $item_use_id = $os->post("item_use_id");
    $branches = $os->get_branches_by_access_name($access_name);
    $usesinfo = $os->mfa($os->mq("SELECT * FROM item_uses WHERE item_use_id='$item_use_id'"));
    ?>
    <input class="uk-hidden" name="item_use_id" value="<?=$item_use_id?>"/>
    <input class="uk-hidden" name="branch_code" value="<?=$os->val($usesinfo,"branch_code")?$os->val($usesinfo,"branch_code"): $selected_branch?>"/>
    ...
    <div class="bp3-form-group">
        ...
        <div class="bp3-form-content">
            <div class="bp3-input-group .modifier">
                ...
                <?
                $dated = $os->val($usesinfo,"dated")?$os->val($usesinfo,"dated"):date("Y-m-d H:i:s");
                ?>
...
if($os->get("get_usesinfo_details_form")=="OK" && $os->post("get_usesinfo_details_form")=="OK"){
    $item_use_id = $os->post("item_use_id");
    $item_uses_detail_id = $os->post("item_uses_detail_id");

    $items_q = $os->mq("SELECT ied.item_uses_detail_id, i.item_name FROM item_uses_details ied 
        INNER JOIN items i ON (i.item_id=ied.item_id AND i.item_type='Product')
        WHERE ied.item_use_id='$item_use_id'");

    $item_entry_detail =
        $os->mfa($os->mq("SELECT ied.*, i.item_name, i.item_type FROM item_uses_details ied 
        INNER JOIN items i ON (i.item_id=ied.item_id)
        
        WHERE ied.item_uses_detail_id='$item_uses_detail_id'"));


    ?>
        <input type="text" class="uk-hidden" name="item_use_id" value="<?=$item_use_id?>">
        <input type="text" class="uk-hidden" name="item_uses_detail_id" value="<?=$item_uses_detail_id?>">
    <div class="uk-grid uk-child-width-1-3 uk-grid-small" uk-grid>
	  ...
	  <div >
            <div class="bp3-form-group .modifier">
                ...
                <div class="bp3-form-content">
                    <div class="bp3-input-group">
                        <input   type="text"
                               class="bp3-input"
                               name="item_stock_info_data"
							   id="item_stock_info_data"
                              />
                ...
                <div class="bp3-form-content">
                    <div class="bp3-input-group">
                        <input   type="number"
                               ...
                               name="quantity"
							   id="quantity_entry"
                               onkeyup="calculate_amount('WT_usesinfo_details_form_modal')"
        ...
        <div style="display:none">   <!--  dosplay none -->
            <div class="bp3-form-group .modifier">
                <label class="bp3-label" for="form-group-input">
                    Rate
                    <span class="bp3-text-muted">*</span>
        ...
        <div class="uk-width-1-1"  style="display:none">   <!--  dosplay none -->
            ...
            while ($item = $os->mfa($items_q)){ ?>
                <label class="bp3-control bp3-checkbox">
                    <input type="checkbox" value="<?=$item["item_uses_detail_id"]?>"
                           name="for_items[]"
                            <?=in_array($item["item_uses_detail_id"], $for_items)?"checked='checked'":""?>
                            />
    ...
    <script>
        $("#insert_usesinfo_details_form #search_item_name").selectize({
            valueField: 'id',
            labelField: 'text',
            searchField: 'text',
            create: false,
            ...
            onChange  : function (value){
                document.querySelector('#insert_usesinfo_details_form #item_id').value = value;
				...
				if(value)
					{
					 $.ajax({
							url: '<?=$ajaxFilePath?>?item_stock_info=OK',
							type: 'GET',
							dataType: 'html',
							data: {
								item_id_val: value, 
							...
							os.setVal('item_stock_info_data',res);
        ...
        $("#insert_usesinfo_details_form #search_brand_name").selectize({
            valueField: 'id',
            labelField: 'text',
            searchField: 'text',
            ...
            sortField: 'text',
            <?if($item_uses_detail_id && $os->val($item_entry_detail, 'brand_id')){?>
            ...
            onChange  : function (value){
                document.querySelector('#insert_usesinfo_details_form input[name=brand_id]').value = value;
            },
            load: function(query, callback) {
                if (!query.length) return callback();
                $.ajax({
        ...
        $("#insert_usesinfo_details_form #search_vendor_name").selectize({
            valueField: 'id',
            labelField: 'text',
            searchField: 'text',
            create: true,
            sortField: 'text',
            <?if($item_uses_detail_id && $os->val($item_entry_detail, 'vendor_id')){?>
            ...
            onChange  : function (value){
                document.querySelector('#insert_usesinfo_details_form input[name=vendor_id]').value = value;
            },
            load: function(query, callback) {
                if (!query.length) return callback();
                $.ajax({
...
if($os->get("get_usesinfo_details_transfer_form")=="OK" && $os->post("get_usesinfo_details_transfer_form")=="OK"){
    $item_use_id = $os->post("item_use_id");
    $item_uses_detail_id = $os->post("item_uses_detail_id");
    $branches = $os->get_branches_by_access_name($access_name);
    ...
    $stock = $os->mfa($os->mq("SELECT 
        ...
        ied.rate
    FROM item_uses_details ied 
    LEFT JOIN item_uses_details ied_t ON ied.item_uses_detail_id= ied_t.parent
    WHERE ied.item_uses_detail_id=$item_uses_detail_id"));
    ?>
                ...
                <div class="bp3-form-content">
                    <div class="bp3-input-group .modifier">
                        <input id="form-group-input" type="number" :modifier
                               class="bp3-input"
                               name="quantity"
                               onkeyup="calculate_amount('WT_usesinfo_details_transfer_form_modal')"
 ...
 * Form Action
 */
if($os->get("insert_edit_usesinfo")=="OK"){
    $item_use_id=$os->post("item_use_id");
    $dated=$os->post("dated");
    $reff_no=$os->post("reff_no");
    $branch_code=$os->post("branch_code");

    $res = ["error"=>true, "redirect"=> $item_use_id<=0];

    if ($reff_no=="" && $item_use_id<=0){
    ...
    if($item_use_id>0){
        $res["error"]=false;
        $res["i_id"] = $item_use_id;
    }
    header('Content-Type: application/json');
    print json_encode($res);
}
if($os->get("insert_edit_usesinfo_details")=="OK"){
    $item_use_id=$os->post("item_use_id");
    $item_uses_detail_id=$os->post("item_uses_detail_id");
    ...
    $res = ["error"=>true];
    if($item_uses_detail_id>0){
	  $update_tracking_table=1;
	  if($update_tracking_table)
	  {
	     // update tracking table 
	    $delete_query="DELETE from item_tracking where item_uses_detail_id='$item_uses_detail_id' ";
		$os->mq($delete_query);
		 $quantity_to_process=$quantity;
		$data=array();
	    $stock_query="select  ied.item_entry_id,ied.item_entry_detail_id,ied.rate, ied.brand_id,	
							group_concat(ied.product_uids) product_uids_in,
							group_concat(it.product_uids) product_uids_out,
							ied.quantity   qty_in,
							SUM(IF(it.quantity>0, it.quantity, 0))   qty_out,
							(ied.quantity - SUM(IF(it.quantity>0, it.quantity, 0))) qty_stock  	
							from  item_entry_details ied 
							INNER JOIN  item_entries  ie on (ied.item_entry_id=ie.item_entry_id)  
							LEFT JOIN  item_tracking  it on (ied.item_entry_detail_id=it.item_entry_detail_id)   
							where  ied.item_id	='$item_id' and  ie.branch_code='$branch_code' and ie.department='$department'   
							group by ied.item_entry_detail_id ";
		
		$stock_query_rs=$os->mq($stock_query);
	    while( $record_stock=$os->mfa( $stock_query_rs))
		{
				
				 
				
				$qty_stock=$record_stock['qty_stock'];
				$item_entry_detail_id=$record_stock['item_entry_detail_id'];
				$rate=$record_stock['rate'];
				$product_uids='';
				 
				 $quantity_to_insert=0;
				if( $qty_stock - $quantity_to_process >=0 )
				{
				   $quantity_to_insert=$quantity_to_process;
				   $quantity_to_process=0;
				   
				}else
				{
				   $quantity_to_insert=$qty_stock;
				   $quantity_to_process=$quantity_to_process-$qty_stock;
				...
				$datatosave_track = array(
				
				"item_id"     =>$item_id,
				"item_entry_detail_id"           =>$item_entry_detail_id,
		       ...
		       $os->save("item_tracking", $datatosave_track, 'item_tracking_id', $item_tracking_id);
	  ...
	  // update tracking table 
        ...
        $res["error"]=false;
        $res["i_id"] = $item_use_id;
        $res["i_d_id"] = $item_uses_detail_id;
    }
    header('Content-Type: application/json');
    print json_encode($res);
}
 ...
 * Delete
 */
if($os->get("WT_delete_usesinfo_details")=="OK" && $os->post("WT_delete_usesinfo")=="OK"){
    $item_use_id = $os->post("item_use_id");
    $item_uses_detail_ids = [];
    $items_query = $os->mfa("SELECT item_uses_detail_id FROM item_uses_details WHERE item_use_id='$item_use_id'");
    while($item = $os->mfa($items_query)){
        $item_uses_detail_ids[] = $item["item_uses_detail_id"];
    }
    $item_uses_detail_ids = "'".implode("', '", $item_uses_detail_ids)."'";
  //  _d($item_uses_detail_ids);
    exit();
}
...
if($os->get("WT_do_verification")=="OK" && $os->post("WT_do_verification")=="OK"){
    ...
    print $os->save("item_uses", $datatosave, "item_use_id", $item_use_id);
}

if($os->get("item_stock_info")=="OK")
{
  
    $item_id=$os->get("item_id_val"); 
	//$item_id=11;
	$item_use_id=$os->get("item_use_id");
	$item_uses_info=$os->rowByField('','item_uses','item_use_id',$item_use_id);
	$branch_code=$item_uses_info['branch_code'];  // need to check this
	 $current_stock=0;
	   
	 
	 
							
			/* $sell_qty="select  SUM(IF(iud.quantity>0, iud.quantity, 0)) from item_uses_details iud where   iud.item_id	='$item_id' group by iud.item_id ";		
			  $sell_qty=0;
			 $stock_query="select        (SUM(ied.quantity)  -    $sell_qty     )   qty_stock   
	  
							from  item_entry_details  ied 
							INNER JOIN item_entries  ie  on (ied.item_entry_id=ie.item_entry_id)  
							where  ied.item_id	='$item_id'  and  ie.branch_code='$branch_code' and ie.department='$department' 
							group by ied.item_id";*/
	   ...
	   $stock_query_rs=$os->mq($stock_query);
	    ...
	    while( $record=$os->mfa( $stock_query_rs))
		{
		    
			 
		   $current_stock =$current_stock+$record['qty_stock'];
		}
	 
	 //  //if(isset($record['qty_stock']))
	  // {
	   //  $current_stock = $record['qty_stock'];
	  // }
