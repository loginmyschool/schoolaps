...
/*
   # wtos version : 1.1
   # page called by ajax script in mess_vendorDataView.php 
   #  
*/
...
if($os->get('WT_mess_vendorListing')=='OK'){
		$where='';
		...
		$listingQuery="  select * from mess_vendor where mess_vendor_id>0   $where   $andvendor_name  $andmobile  $andemail  $andaddress  $andactive_status  $andbranch_code  $andnote  $andaddedBy   order by mess_vendor_id desc";
...
<table  border="0" cellspacing="0" cellpadding="0" class="noBorder"  >
							...
							<tr class="trListing">
									...
									<td> 
									<? if($os->access('wtView')){ ?>
									<span  class="actionLink" ><a href="javascript:void(0)"  onclick="WT_mess_vendorGetById('<? echo $record['mess_vendor_id'];?>')" >Edit</a></span>  <? } ?>  </td>

									<td><?php echo $record['vendor_name']?> </td>  
									<td><?php echo $record['mobile']?> </td>  
									<td><?php echo $record['email']?> </td>  
									<td><?php echo $record['address']?> </td>  
									<td> <? if(isset($os->mess_vendor_active_status[$record['active_status']])){ echo  $os->mess_vendor_active_status[$record['active_status']]; } ?></td> 
									<td>  <? echo 
...
if($os->get('WT_mess_vendorEditAndSave')=='OK')
{
 $mess_vendor_id=$os->post('mess_vendor_id');
 ...
 $dataToSave['note']=addslashes($os->post('note')); 
		...
		$dataToSave['addedBy']=$os->userDetails['adminId'];
          ...
          $qResult=$os->save('mess_vendor',$dataToSave,'mess_vendor_id',$mess_vendor_id);///    allowed char '\*#@/"~$^.,()|+_-=:££ 	
		if($qResult)  
				{
		if($mess_vendor_id>0 ){ $mgs= " Data updated Successfully";}
		if($mess_vendor_id<1 ){ $mgs= " Data Added Successfully"; $mess_vendor_id=  $qResult;}
		
		  $mgs=$mess_vendor_id."#-#".$mgs;
		}
		else
		{
...
if($os->get('WT_mess_vendorGetById')=='OK')
{
		$mess_vendor_id=$os->post('mess_vendor_id');
		
		if($mess_vendor_id>0)	
		{
		$wheres=" where mess_vendor_id='$mess_vendor_id'";
		}
	    $dataQuery=" select * from mess_vendor  $wheres ";
...
if($os->get('WT_mess_vendorDeleteRowById')=='OK')
{ 

$mess_vendor_id=$os->post('mess_vendor_id');
 if($mess_vendor_id>0){
 $updateQuery="delete from mess_vendor where mess_vendor_id='$mess_vendor_id'";
 $os->mq($updateQuery);
