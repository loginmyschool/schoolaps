...
/*
   # wtos version : 1.1
   # main ajax process page : budget_list_detailsAjax.php 
   #  
*/
...
?><?
$pluginName='';
$listHeader='Budget Details List';
$ajaxFilePath= 'budget_list_detailsAjax.php';
...
?>
 ...
 <table class="container">
  ...
  <tr>
	...
	<? if($os->access('wtDelete')){ ?><input type="button" value="Delete" onclick="WT_budget_list_detailsDeleteRowById('');" /><? } ?>	 
	&nbsp;&nbsp;
	&nbsp; <input type="button" value="New" onclick="javascript:window.location='';" />
	 
	&nbsp;<? if($os->access('wtEdit')){ ?> <input type="button" value="Save" onclick="WT_budget_list_detailsEditAndSave();" /><? } ?>	 
...
<tr >
										...
										<td> <select name="budget_list_id" id="budget_list_id" class="textbox fWidth" ><option value="">Select Budget Purpose</option>		  	<? 
								
										  $os->optionsHTML('','budget_list_id','title','budget_list');?>
							</select> </td>						
										</tr><tr >
	...
	<input type="hidden"  id="budget_list_details_id" value="0" />	
	<input type="hidden"  id="WT_budget_list_detailspagingPageno" value="1" />	
	<div class="formDivButton">						
	<? if($os->access('wtDelete')){ ?><input type="button" value="Delete" onclick="WT_budget_list_detailsDeleteRowById('');" />	<? } ?>	  
	&nbsp;&nbsp;
	&nbsp; <input type="button" value="New" onclick="javascript:window.location='';" />
 ...
 Budget Purpose:
	...
	<select name="budget_list_id" id="budget_list_id_s" class="textbox fWidth" ><option value="">Select Budget Purpose</option>		  	<? 
  ...
  <input type="button" value="Search" onclick="WT_budget_list_detailsListing();" style="cursor:pointer;"/>
  <input type="button" value="Reset" onclick="searchReset();" style="cursor:pointer;"/>
  
   </div>
	<div  class="ajaxViewMainTableTDListData" id="WT_budget_list_detailsListDiv">&nbsp; </div>
	&nbsp;</td>
  </tr>
</table>
...
function WT_budget_list_detailsListing() // list table searched data get 
{
	var formdata = new FormData();
	
	
 var budget_list_id_sVal= os.getVal('budget_list_id_s'); 
...
formdata.append('budget_list_id_s',budget_list_id_sVal );
...
formdata.append('status_s',status_sVal );
	...
	var WT_budget_list_detailspagingPageno=os.getVal('WT_budget_list_detailspagingPageno');
	var url='wtpage='+WT_budget_list_detailspagingPageno;
	url='<? echo $ajaxFilePath ?>?WT_budget_list_detailsListing=OK&'+url;
...
WT_budget_list_detailsListing();
function  searchReset() // reset Search Fields
	{
		 os.setVal('budget_list_id_s',''); 
...
var statusVal= os.getVal('status'); 
 ...
 formdata.append('status',statusVal );
	 ...
	 formdata.append('budget_list_details_id',budget_list_details_id );
  	var url='<? echo $ajaxFilePath ?>?WT_budget_list_detailsEditAndSave=OK&';
	os.animateMe.div='div_busy';
	os.animateMe.html='<div class="loadImage"><img  src="<? echo $loadingImage?>"  /> <div class="loadText">&nbsp;Please wait. Working...</div></div>';	
...
function WT_budget_list_detailsReLoadList(data) // after edit reload list
{
	...
	var d=data.split('#-#');
	var budget_list_details_id=parseInt(d[0]);
	if(d[0]!='Error' && budget_list_details_id>0)
	{
	  os.setVal('budget_list_details_id',budget_list_details_id);
	}
...
function WT_budget_list_detailsGetById(budget_list_details_id) // get record by table primery id
{
	var formdata = new FormData();	 
	formdata.append('budget_list_details_id',budget_list_details_id );
	var url='<? echo $ajaxFilePath ?>?WT_budget_list_detailsGetById=OK&';
	os.animateMe.div='div_busy';
	os.animateMe.html='<div class="loadImage"><img  src="<? echo $loadingImage?>"  /> <div class="loadText">&nbsp;Please wait. Working...</div></div>';	
	os.setAjaxFunc('WT_budget_list_detailsFillData',url,formdata);
				
}

function WT_budget_list_detailsFillData(data)  // fill data form by JSON
{
   
	var objJSON = JSON.parse(data);
	os.setVal('budget_list_details_id',parseInt(objJSON.budget_list_details_id));
	
 os.setVal('budget_list_id',objJSON.budget_list_id); 
...
function WT_budget_list_detailsDeleteRowById(budget_list_details_id) // delete record by table id
{
	var formdata = new FormData();	 
	...
	var url='<? echo $ajaxFilePath ?>?WT_budget_list_detailsDeleteRowById=OK&';
	os.animateMe.div='div_busy';
	os.animateMe.html='<div class="loadImage"><img  src="<? echo $loadingImage?>"  /> <div class="loadText">&nbsp;Please wait. Working...</div></div>';	
	os.setAjaxFunc('WT_budget_list_detailsDeleteRowByIdResults',url,formdata);
	}
 

}
function WT_budget_list_detailsDeleteRowByIdResults(data)
{
	alert(data);
	WT_budget_list_detailsListing();
} 
...
{
	os.setVal('WT_budget_list_detailspagingPageno',parseInt(pageNo));
	WT_budget_list_detailsListing();
}
