...
/*
   # wtos version : 1.1
   # main ajax process page : global_vendorAjax.php 
   #  
*/
...
$ajaxFilePath= 'global_vendorAjax.php';
...
?>
 ...
 <table class="container">
  ...
  <tr>
	...
	<? if($os->access('wtDelete')){ ?><input type="button" value="Delete" onclick="WT_global_vendorDeleteRowById('');" /><? } ?>	 
	&nbsp;&nbsp;
	&nbsp; <input type="button" value="New" onclick="javascript:window.location='';" />
	 
	&nbsp;<? if($os->access('wtEdit')){ ?> <input type="button" value="Save" onclick="WT_global_vendorEditAndSave();" /><? } ?>	 
...
<tr >
	...
	<select name="active_status" id="active_status" class="textbox fWidth" ><option value="">Select Active Status</option>	<? 
										  $os->onlyOption($os->global_vendor_active_status);	?></select>	 </td>						
										</tr><tr >
	  									<td>Branch Code </td>
	...
	<input type="hidden"  id="global_vendor_id" value="0" />	
	<input type="hidden"  id="WT_global_vendorpagingPageno" value="1" />	
	<div class="formDivButton">						
	<? if($os->access('wtDelete')){ ?><input type="button" value="Delete" onclick="WT_global_vendorDeleteRowById('');" />	<? } ?>	  
	&nbsp;&nbsp;
	&nbsp; <input type="button" value="New" onclick="javascript:window.location='';" />
 ...
 Vendor Name: <input type="text" class="wtTextClass" name="vendor_name_s" id="vendor_name_s" value="" /> &nbsp;  Mobile: <input type="text" class="wtTextClass" name="mobile_s" id="mobile_s" value="" /> &nbsp;  Email: <input type="text" class="wtTextClass" name="email_s" id="email_s" value="" /> &nbsp;  Address: <input type="text" class="wtTextClass" name="address_s" id="address_s" value="" /> &nbsp;  Active Status:
	...
	<select name="active_status" id="active_status_s" class="textbox fWidth" ><option value="">Select Active Status</option>	<? 
										  $os->onlyOption($os->global_vendor_active_status);	?></select>	
   Branch Code:
  ...
  <input type="button" value="Search" onclick="WT_global_vendorListing();" style="cursor:pointer;"/>
  <input type="button" value="Reset" onclick="searchReset();" style="cursor:pointer;"/>
  
   </div>
	<div  class="ajaxViewMainTableTDListData" id="WT_global_vendorListDiv">&nbsp; </div>
	&nbsp;</td>
  </tr>
</table>
...
function WT_global_vendorListing() // list table searched data get 
{
		...
		var WT_global_vendorpagingPageno=os.getVal('WT_global_vendorpagingPageno');
		var url='wtpage='+WT_global_vendorpagingPageno;
		url='<? echo $ajaxFilePath ?>?WT_global_vendorListing=OK&'+url;
		os.animateMe.div='div_busy';
		os.animateMe.html='<div class="loadImage"><img  src="<? echo $loadingImage?>"  /> <div class="loadText">&nbsp;Please wait. Working...</div></div>';	
		os.setAjaxHtml('WT_global_vendorListDiv',url,formdata);
		
}

WT_global_vendorListing();
...
function WT_global_vendorEditAndSave(){
		...
		var   global_vendor_id=os.getVal('global_vendor_id');
		formdata.append('global_vendor_id',global_vendor_id );
		var url='<? echo $ajaxFilePath ?>?WT_global_vendorEditAndSave=OK&';
		os.animateMe.div='div_busy';
		os.animateMe.html='<div class="loadImage"><img  src="<? echo $loadingImage?>"  /> <div class="loadText">&nbsp;Please wait. Working...</div></div>';	
		os.setAjaxFunc('WT_global_vendorReLoadList',url,formdata);

}	

function WT_global_vendorReLoadList(data) // after edit reload list
{
  
	var d=data.split('#-#');
	var global_vendor_id=parseInt(d[0]);
	if(d[0]!='Error' && global_vendor_id>0)
	{
	  os.setVal('global_vendor_id',global_vendor_id);
	}
...
function WT_global_vendorGetById(global_vendor_id) // get record by table primery id
{
	var formdata = new FormData();	 
	formdata.append('global_vendor_id',global_vendor_id );
	var url='<? echo $ajaxFilePath ?>?WT_global_vendorGetById=OK&';
	os.animateMe.div='div_busy';
	os.animateMe.html='<div class="loadImage"><img  src="<? echo $loadingImage?>"  /> <div class="loadText">&nbsp;Please wait. Working...</div></div>';	
	os.setAjaxFunc('WT_global_vendorFillData',url,formdata);
				
}

function WT_global_vendorFillData(data){
		var objJSON = JSON.parse(data);
		os.setVal('global_vendor_id',parseInt(objJSON.global_vendor_id));
...
function WT_global_vendorDeleteRowById(global_vendor_id) // delete record by table id
{
	var formdata = new FormData();	 
	if(parseInt(global_vendor_id)<1 || global_vendor_id==''){  
	var  global_vendor_id =os.getVal('global_vendor_id');
	}
	
	if(parseInt(global_vendor_id)<1){ alert('No record Selected'); return;}
	...
	var url='<? echo $ajaxFilePath ?>?WT_global_vendorDeleteRowById=OK&';
	os.animateMe.div='div_busy';
	os.animateMe.html='<div class="loadImage"><img  src="<? echo $loadingImage?>"  /> <div class="loadText">&nbsp;Please wait. Working...</div></div>';	
	os.setAjaxFunc('WT_global_vendorDeleteRowByIdResults',url,formdata);
	}
 

}
function WT_global_vendorDeleteRowByIdResults(data)
{
	alert(data);
	WT_global_vendorListing();
} 
...
{
	os.setVal('WT_global_vendorpagingPageno',parseInt(pageNo));
	WT_global_vendorListing();
}
</script>
