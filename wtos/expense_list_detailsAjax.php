...
/*
   # wtos version : 1.1
   # page called by ajax script in expense_list_detailsDataView.php 
   #  
*/
...
$andexpense_list_id=  $os->postAndQuery('expense_list_id_s','expense_list_id','%');
...
<td ><b>Expense Purpose</b></td>  
  ...
  <td ><b>Status</b></td>  
							...
							<td> 
							<? if($os->access('wtView')){ ?>
							<span  class="actionLink" ><a href="javascript:void(0)"  onclick="WT_expense_list_detailsGetById('<? echo $record['expense_list_details_id'];?>')" >Edit</a></span>  <? } ?>  </td>
...
if($os->get('WT_expense_list_detailsEditAndSave')=='OK')
{
 $expense_list_details_id=$os->post('expense_list_details_id');
 
 
		 
 $dataToSave['expense_list_id']=addslashes($os->post('expense_list_id')); 
 ...
 $dataToSave['status']=addslashes($os->post('status')); 
		...
		$dataToSave['addedBy']=$os->userDetails['adminId'];
          ...
          $qResult=$os->save('expense_list_details',$dataToSave,'expense_list_details_id',$expense_list_details_id);///    allowed char '\*#@/"~$^.,()|+_-=:££ 	
		if($qResult)  
				{
		if($expense_list_details_id>0 ){ $mgs= " Data updated Successfully";}
		if($expense_list_details_id<1 ){ $mgs= " Data Added Successfully"; $expense_list_details_id=  $qResult;}
		
		  $mgs=$expense_list_details_id."#-#".$mgs;
		}
		else
		{
...
if($os->get('WT_expense_list_detailsGetById')=='OK')
{
		$expense_list_details_id=$os->post('expense_list_details_id');
		
		if($expense_list_details_id>0)	
		{
		$wheres=" where expense_list_details_id='$expense_list_details_id'";
		}
	    $dataQuery=" select * from expense_list_details  $wheres ";
		$rsResults=$os->mq($dataQuery);
		$record=$os->mfa( $rsResults);
		
		 
 $record['expense_list_id']=$record['expense_list_id'];
...
if($os->get('WT_expense_list_detailsDeleteRowById')=='OK')
{ 
