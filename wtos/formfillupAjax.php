         ...
         <div class="listingRecords">
            ...
            <table  border="0" cellspacing="0" cellpadding="0" class="noBorder"  >
               ...
               while($record=$os->mfa( $rsRecords)){ 
                  ...
                  ?>
                  <tr class="trListing">
                     <td title="ID:<?php echo $record['formfillup_id']?>"><?php echo $serial; ?>     </td>
                     <td class="uk-text-nowrap"> 
                        ...
                        <td><?php echo $os->classList[$record['class_id']]?> </td>
               ...
               if($os->get('WT_formfillupEditAndSave')=='OK'){
                  ...
                  if($os->post('formfillup_id')=='0'){
                     ...
                     $formfillup_id=$os->rowByField('formfillup_id','formfillup','form_no',$os->post('form_no'));
                     if($formfillup_id>0){
                        echo $mgs="Error#-#Form no Already exist.";
                        return;
                     }
                  }
                  ...
                  $dataToSave['mother_monthly_income']=$os->post('mother_monthly_income')?$os->post('mother_monthly_income'):0.00;  
                  ...
                  $dataToSave['physically_challanged']=addslashes($os->post('physically_challanged')); 
                  ...
                  $dataToSave['application_no']=addslashes($os->post('application_no'));
                  if($os->post('application_date')){
                     $application_date_a=explode('-',$os->post('application_date'));
                     $dataToSave['application_date']=$os->saveDate($application_date_a[2].'-'.$application_date_a[1].'-'.$application_date_a[0]); 
                  }
                  if($os->post('exam_date')){
                     $exam_date_a=explode('-',$os->post('exam_date'));
                     $dataToSave['exam_date']=$os->saveDate($exam_date_a[2].'-'.$exam_date_a[1].'-'.$exam_date_a[0]); 
                  }
                  $dataToSave['exam_time']=addslashes($os->post('exam_time'));
                  $dataToSave['exam_center']=addslashes($os->post('exam_center'));
                  $dataToSave['center_code']=addslashes($os->post('center_code'));
                  ...
                  if($os->get('WT_formfillupGetById')=='OK'){
                     ...
                     if($record['profile_picture']!=''){
                        ...
                        $record['application_date']=$os->showDate($record['application_date']); 
                        $record['exam_date']=$os->showDate($record['exam_date']); 
