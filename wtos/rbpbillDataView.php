...
/*
   # wtos version : 1.1
   # main ajax process page : rbpbillAjax.php
   #
*/
...
?><?
$pluginName='';
$listHeader='Manage Bill';
$ajaxFilePath= 'rbpbillAjax.php';
...
?>
 ...
 <table class="container"  cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	<? if($os->access('wtDelete')){ ?><input type="button" value="Delete" onclick="WT_rbpbillDeleteRowById('');" /><? } ?>
	&nbsp;&nbsp;
	&nbsp; <input type="button" value="New" onclick="javascript:window.location='';" />

	&nbsp;<? if($os->access('wtEdit')){ ?> <input type="button" value="Save" onclick="WT_rbpbillEditAndSave();" /><? } ?>
...
<tr >
                        ...
                        <span id="SAA_Emp" class="SAA_Container"> </span>
                				...
                				saa.execute('e','SAA_Emp','rbemployeeId','rbemployee','rbemployeeId','name,designation,moble,status','Name,Deg,Mob,Status','rbemployeeId','','');
										...
										<tr>
										<td> 
									View|Add products</td> <td >  <span  class="actionLink" ><a href="javascript:void(0)"  onclick="openProducts()"	>Products</a></span>
			      ...
			      //    $os->productsArray= $os->getProductsArray();  	
                     ...
                     $options['foreignId']='rbpbillId';
                     $options['foreignTable']='rbpbill';
                     $options['table']='rbpbilldetails';
                     $options['tableId']='rbpbilldetailsId';
                     $options['tableQuery']="select * from rbpbilldetails where [condition] order by rbpbilldetailsId ";
                  ...
                  //   $options['inlineEdit']=array('details'=>'yes');
					...
					function openProducts()
					{
						...
						var rbpurchaseId=os.getVal('rbpbillId');
					 ...
					 function calculateProductTotal() // for quick edit fields
					 {
					    ...
					    os.setVal('producttotalPrice',producttotalPrice);
										...
										<td><input value="" type="text" name="productAmount" id="productAmount" class="textboxxx  fWidth readonlyStyle"  readonly="readonly" /> 
										<select name="paymentStatus" id="paymentStatus" class="textbox fWidth" ><option value=""> </option>	<?
										...
										<td><input value="" type="text" name="discountedPrice" id="discountedPrice" class="textboxxx  fWidth  readonlyStyle"  readonly="readonly" /> </td>
	  									...
	  									<td>Tax @ <input value="" type="text" name="taxRate" id="taxRate" class="textboxxx  fWidth " onkeyup="calculatePaybleAndDue()" style="width:30px"/> % </td>
										<td><input value="" type="text" name="taxAmount" id="taxAmount" class="textboxxx  fWidth  readonlyStyle"  readonly="readonly"  /> </td>
										</tr><tr >
	  									<td>Delivery Charge </td>
										<td><input value="" type="text" name="deliveryCharge" id="deliveryCharge" class="textboxxx  fWidth " onkeyup="calculatePaybleAndDue()"/> </td>
										</tr><tr >
	  									<td>Install Charge </td>
										<td><input value="" type="text" name="installCharge" id="installCharge" class="textboxxx  fWidth " onkeyup="calculatePaybleAndDue()"/> </td>
										</tr><tr >
	  									<td>Payble Amount </td>
										<td><input value="" type="text" name="paybleAmount" id="paybleAmount" class="textboxxx  fWidth  readonlyStyle"  readonly="readonly" /> </td>
										</tr><tr >
	  									<td>Paid Amount </td>
										<td><input value="" type="text" name="paidAmount" id="paidAmount" class="textboxxx fWidth readonlyStyle"  readonly="readonly" /> 
                    ...
                    <? //  include('quickEditPage.php');
                     ...
                     $options['foreignId']='rbpbillId';
                     $options['foreignTable']='rbpbill';
                     $options['table']='rbpbillpayment';
                     $options['tableId']='rbpbillpaymentId';
                     $options['tableQuery']="select * from rbpbillpayment where [condition] order by rbpbillpaymentId ";
                      ...
                      function setPaymentTotal(data)
                      {
						 ...
						 calculatePaybleAndDue();
										...
										<td><input value="" type="text" name="dueAmount" id="dueAmount" class="textboxxx  fWidth  readonlyStyle"  readonly="readonly" /> </td>
	  									...
	  									<td colspan="2">Remarks <br /> <textarea  name="remarks" id="remarks" style="width:256px; height:37px;" class="textboxxx fWidth"  ></textarea></td>
										</tr>
...
<tr style="display:none;" >
	...
	<input type="hidden"  id="rbpbillId" value="0" />
	<input type="hidden"  id="WT_rbpbillpagingPageno" value="1" />
	<div class="formDivButton">
	<? if($os->access('wtDelete')){ ?><input type="button" value="Delete" onclick="WT_rbpbillDeleteRowById('');" />	<? } ?>
	&nbsp;&nbsp;
	&nbsp; <input type="button" value="New" onclick="javascript:window.location='';" />
...
From Bill Date: <input class="wtDateClass" type="text" name="f_billDate_s" id="f_billDate_s" value=""  /> &nbsp;   To Bill Date: <input class="wtDateClass" type="text" name="t_billDate_s" id="t_billDate_s" value=""  /> &nbsp;
   Contacts:
  ...
  <input type="button" value="Search" onclick="WT_rbpbillListing();" style="cursor:pointer;"/>
  <input type="button" value="Reset" onclick="searchReset();" style="cursor:pointer;"/>

   </div>
	<div  class="ajaxViewMainTableTDListData" id="WT_rbpbillListDiv">
	<? include('quickListing.php'); 
	$listingQuery="  select * from rbpbill order by rbpbillId desc";
	pbill($listingQuery);
	?> 
	</div>
	&nbsp;</td>
...
function WT_rbpbillListing() // list table searched data get
{
...
formdata.append('remarks_s',remarks_sVal );
	...
	var WT_rbpbillpagingPageno=os.getVal('WT_rbpbillpagingPageno');
	var url='wtpage='+WT_rbpbillpagingPageno;
	url='<? echo $ajaxFilePath ?>?WT_rbpbillListing=OK&'+url;
...
function WT_rbpbillEditAndSave()  // collect data and send to save
{
...
if(os.check.empty('remarks','Please Add Remarks')==false){ return false;}
	 ...
	 var   rbpbillId=os.getVal('rbpbillId');
	 formdata.append('rbpbillId',rbpbillId );
  	var url='<? echo $ajaxFilePath ?>?WT_rbpbillEditAndSave=OK&';
	os.animateMe.div='div_busy';
	os.animateMe.html='<div class="loadImage"><img  src="<? echo $loadingImage?>"  /> <div class="loadText">&nbsp;Please wait. Working...</div></div>';
	os.setAjaxFunc('WT_rbpbillReLoadList',url,formdata);

}

function WT_rbpbillReLoadList(data) // after edit reload list
{

	var d=data.split('#-#');
	var rbpbillId=parseInt(d[0]);
	if(d[0]!='Error' && rbpbillId>0)
	{
	  os.setVal('rbpbillId',rbpbillId);
	}
...
function WT_rbpbillGetById(rbpbillId) // get record by table primery id
{
	var formdata = new FormData();
	formdata.append('rbpbillId',rbpbillId );
	var url='<? echo $ajaxFilePath ?>?WT_rbpbillGetById=OK&';
	os.animateMe.div='div_busy';
	os.animateMe.html='<div class="loadImage"><img  src="<? echo $loadingImage?>"  /> <div class="loadText">&nbsp;Please wait. Working...</div></div>';
	os.setAjaxFunc('WT_rbpbillFillData',url,formdata);

}

function WT_rbpbillFillData(data)  // fill data form by JSON
{

	var objJSON = JSON.parse(data);
	os.setVal('rbpbillId',parseInt(objJSON.rbpbillId));
...
ajaxViewpayments(objJSON.rbpbillId);
ajaxViewproduct(objJSON.rbpbillId);

}

function WT_rbpbillDeleteRowById(rbpbillId) // delete record by table id
{
	var formdata = new FormData();
	if(parseInt(rbpbillId)<1 || rbpbillId==''){
	var  rbpbillId =os.getVal('rbpbillId');
	}

	if(parseInt(rbpbillId)<1){ alert('No record Selected'); return;}
	...
	var url='<? echo $ajaxFilePath ?>?WT_rbpbillDeleteRowById=OK&';
	os.animateMe.div='div_busy';
	os.animateMe.html='<div class="loadImage"><img  src="<? echo $loadingImage?>"  /> <div class="loadText">&nbsp;Please wait. Working...</div></div>';
	os.setAjaxFunc('WT_rbpbillDeleteRowByIdResults',url,formdata);
	}


}
function WT_rbpbillDeleteRowByIdResults(data)
{
	alert(data);
	WT_rbpbillListing();
}
...
{
	os.setVal('WT_rbpbillpagingPageno',parseInt(pageNo));
	WT_rbpbillListing();
}
