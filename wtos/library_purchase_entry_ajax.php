...
$department = "Library";
$access_name = "Library Purchase Entry";
//
...
if($os->get("WT_fetch_purchase_details")=="OK"&&$os->post("WT_fetch_purchase_details")=="OK"){
    ...
    <div class="item-content uk-overflow-auto">
        <table class="bp3-html-table  bp3-html-table-striped bp3-interactive bp3-html-table-condensed" style="width: 100%">
            ...
            <tbody>
            <?
            $query = $os->mq("SELECT ied.*, i.item_name, i.item_unit, gb.brand_name, gv.vendor_name
            ...
            while($record=$os->mfa($query)){
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <div class="text-s uk-hidden@s">Quantity : <i><?=rand(1,99)?><?= $record["item_unit"]?></i></div>
                        ...
                        <div class="text-s m-top-s">
                            ...
                            if ($delete_access){
                            ?>
                            <a class="uk-text-danger" tabindex="0" role="button"
                               onclick="removeRowAjaxFunction('item_entry_details','item_entry_detail_id','<?=$record['item_entry_detail_id']?>','','','WT_fetch_purchase_details(\'<?=$item_entry_id?>\')') ">Delete</a>
                            <?}
                            if ($edit_access){
                            ?>
                            \
                    ...
                    <td class="uk-visible@s"><?=$record['quantity']?><?= $record["item_unit"]?></td>
    ...
    <script>
                        ...
                        <div>
                            <div class="uk-grid uk-grid-collapse">
                                ...
                                <div class="uk-width-expand">
                                    <div class="name">${escape(item.text)}(${escape(item.edition)})</div>
        ...
        $("#insert_purchase_details_form #search_brand_name").selectize({
            ...
            create:function (input, callback){
                $.ajax({
                    url: '<?=$site['url-wtos']?>wtosAjax.php?insert_brand=OK&brand='+input,
                    type: 'GET',
        ...
        $("#insert_purchase_details_form #search_vendor_name").selectize({
            ...
            create:function (input, callback){
                $.ajax({
                    url: '<?=$site['url-wtos']?>wtosAjax.php?insert_vendor=OK&vendor='+input+'&branch_code=<?=$selected_branch?>',
                    type: 'GET',
                    success: function (result) {
                        result = JSON.parse(result);
                        if (result) {
...
if($os->get("insert_edit_purchase_details")=="OK"){
    ...
    //item details
    $item = $os->mfa($os->mq("SELECT * FROM items WHERE item_id='$item_id'"));
    $is_generate_barcode = $item["barcode_applicable"];
    ///
    ...
    $generated_id = $os->save("item_entry_details", $datatosave, 'item_entry_detail_id', $item_entry_detail_id);

    $res = ["error"=>true];
    if($generated_id>0){
        $res["error"]=false;
        $res["i_id"] = $item_entry_id;
        $res["i_d_id"] = $generated_id;
        //for new item
        if($item_entry_detail_id<=0 && $is_generate_barcode=="Yes"){
            $i_u_ids = [];
            for($x=0;$x<$quantity;$x++) {
                $datatosave = [
                    "item_entry_detail_id" => $generated_id,
                    "item_id" => $item_id
                ];
                $i_u_id = $os->save("item_unique", $datatosave, "item_unique_id", "");
                $i_u_ids[] = $i_u_id;
            }
            $i_u_ids = implode(",",$i_u_ids);
            $datatosave = [
                    "product_uids"=> $i_u_ids
            ];
            $os->save("item_entry_details", $datatosave, 'item_entry_detail_id', $generated_id);
