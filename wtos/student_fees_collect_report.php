...
$listHeader='Fees Received Report';

$ajaxFilePath= 'student_fees_collect_report_ajax.php';
...
// fees_student due amount only for show. main due calculation from payment // not need at the moment caculation change in calculate_due function
   

include($site['root-wtos'].'admission_admin_function_helpers.php');
//$due_calculation="select * from fees_student where paymentStatus='installment' ";
//$due_calculation_rs= $os->mq($due_calculation);
//while ($row = $os->mfa($due_calculation_rs))
//{
	// $due= calculate_due($row['fees_student_id'],0,$row['paymentStatus']);
	/// $dataToSave=array();
	// $dataToSave['currentDueAmount']=$due;
	// $fees_student_id=$os->save('fees_student',$dataToSave,'fees_student_id',$row['fees_student_id']);
	  
	 //fees_student  fees_payment_ids nedd to update  // add comma at last

//}


 /*   date hasbeen updated
 
  $update_q="select * from fees_student where dated='0000-00-00 00:00:00' ";
$update_q_rs= $os->mq($update_q);
while ($row = $os->mfa($update_q_rs))
{
  
 $month=$row['month'];
 $month=str_pad($month, 2, "0", STR_PAD_LEFT);
  $year=$row['year'];
  $dated=$year.'-'.$month.'-01 00:00:00';
...
<div class="content">
    <div class="item">
		...
		<select name="branch_code_s" id="branch_code_s"
		 ...
		 <input type="text" id="from_date" class="wtDateClass textbox fWidth" style="width:150px;" placeholder="yyyy-mm-dd" value="<? echo date('Y-m-d'); ?>" /> 
		 
		
		 </div>
		  <div class="uk-inline uk-margin-small-left" uk-tooltip=""> To 
		  <input type="text" id="to_date" class="wtDateClass textbox fWidth" style="width:150px;"  placeholder="yyyy-mm-dd"  value="<? echo date('Y-m-d'); ?>" />
            ...
            <button style="display:none;" class="uk-button uk-border-rounded  uk-button-small uk-secondary-button  uk-flex-inline uk-flex-middle" type="button" onclick="searchReset();">
...
<script>
    ...
    function monthly_fees_report() // get record by table primery id
    {
		...
		var from_date=os.getVal('from_date');
        formdata.append('from_date',from_date );
		
		var to_date=os.getVal('to_date');
		...
		if(os.check.empty('from_date','Please select date from')==false){ return false;}
		if(os.check.empty('to_date','Please select date to')==false){ return false;}
